...
const getDatasetCount = () => {
  const { result } = simnet.callReadOnlyFn("atmos-v4", "get-dataset-count", [], deployer);
  return uintFromResponse(result);
};

const getRewardBalance = (owner: string) => {
  const { result } = simnet.callReadOnlyFn("atmos-token-v4", "get-balance", [Cl.principal(owner)], deployer);
  return uintFromResponse(result);
};

const getStakeAmount = (owner: string) => {
  const { result } = simnet.callReadOnlyFn("atmos-staking-v4", "get-stake-info", [Cl.principal(owner)], deployer);
...
describe("atmos-v4 contract tests", () => {
  ...
  it("should return contract admin", () => {
    const { result } = simnet.callReadOnlyFn("atmos-v4", "get-contract-admin", [], deployer);
  ...
  it("should reject direct reward mint calls from users", () => {
    const { result } = simnet.callPublicFn("atmos-token-v4", "mint-registration-reward", [
  ...
  it("should reject direct staking reward mint calls from users", () => {
    const { result } = simnet.callPublicFn("atmos-token-v4", "mint-staking-reward", [
      Cl.principal(wallet1),
      Cl.uint(REGISTER_REWARD_AMOUNT),
    ], wallet1);
  ...
  it("should allow staking ATMOS tokens", () => {
    ...
    const { result } = simnet.callPublicFn("atmos-staking-v4", "stake", [
      Cl.uint(500_000),
    ], wallet3);
  ...
  it("should accrue and allow claiming APY rewards", () => {
    ...
    simnet.callPublicFn("atmos-staking-v4", "stake", [Cl.uint(900_000)], wallet4);
    simnet.mineEmptyBurnBlocks(2000);
    const before = getRewardBalance(wallet4);

    const claim = simnet.callPublicFn("atmos-staking-v4", "claim-rewards", [], wallet4);
  ...
  it("should allow unstaking tokens", () => {
    ...
    simnet.callPublicFn("atmos-staking-v4", "stake", [Cl.uint(300_000)], wallet2);
    const beforeStake = getStakeAmount(wallet2);
    const beforeBalance = getRewardBalance(wallet2);

    const { result } = simnet.callPublicFn("atmos-staking-v4", "unstake", [
      Cl.uint(200_000),
    ], wallet2);
  ...
  it("should retrieve dataset information", () => {
    ...
    const { result } = simnet.callReadOnlyFn("atmos-v4", "get-dataset", [Cl.uint(id)], deployer);
    const tuple = tupleFromResponse(result);
  ...
  it("should return datasets by owner", () => {
    ...
    const { result } = simnet.callReadOnlyFn("atmos-v4", "get-datasets-by-owner", [Cl.principal(wallet2)], deployer);
  ...
  it("should allow owner to update dataset metadata", () => {
    ...
    const { result } = simnet.callPublicFn("atmos-v4", "update-dataset-metadata", [
      Cl.uint(id),
  ...
  it("should allow owner to freeze dataset metadata", () => {
    ...
    const { result } = simnet.callPublicFn("atmos-v4", "freeze-dataset-metadata", [Cl.uint(id)], wallet1);
    expect(result).toBeOk(Cl.bool(true));
  });

  it("should allow admin to set validator", () => {
    const { result } = simnet.callPublicFn("atmos-v4", "set-validator", [
  ...
  it("should allow validator to verify dataset", () => {
    ...
    simnet.callPublicFn("atmos-v4", "set-validator", [Cl.principal(wallet2), Cl.bool(true)], deployer);

    const verify = simnet.callPublicFn("atmos-v4", "verify-dataset", [Cl.uint(id)], wallet2);
  ...
  it("should reject verification by non-validator", () => {
    ...
    const { result } = simnet.callPublicFn("atmos-v4", "verify-dataset", [Cl.uint(id)], wallet2);
  ...
  it("should allow validator to reject dataset", () => {
    ...
    const reject = simnet.callPublicFn("atmos-v4", "reject-dataset", [Cl.uint(id)], wallet2);
